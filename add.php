...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $errors[] = "Год должен быть числом.";
    }

    if (empty($errors)) {
        $stmt = $mysqli->prepare("INSERT INTO cars (title, description, model, license_plate, year) VALUES (?, ?, ?, ?, ?)");
        $yearParam = $year === '' ? null : (int)$year;
        $stmt->bind_param("ssssi", $title, $description, $model, $license_plate, $yearParam);
        if ($stmt->execute()) {
